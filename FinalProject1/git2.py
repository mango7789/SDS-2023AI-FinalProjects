...
class Node:
    ...
    def expand(self):
        for col in self.board.get_valid_moves():
            self.children["valid_actions"] = Node(self.visits, self.board.drop_piece(col), self.player%2+1)
...
class MCTS:
    ...
    def get_best_action(self):
        ...
        if self.root.children != {}:
            ...
            return random.choice([c for c in range(CFG_COLS) if not self.root.board.board[c]] )
...
print("My Agent vs Random Agent:", mean_reward(evaluate("connectx", [sp_mcts_agent, "random"], num_episodes=10)))
print("My Agent vs Negamax Agent:", mean_reward(evaluate("connectx", [sp_mcts_agent, "negamax"], num_episodes=10)))